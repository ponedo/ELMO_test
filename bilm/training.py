...
class LanguageModel(object):
    ...
    def _build_word_embeddings(self):
        ...
        with tf.device("/cpu:0"):
            ############################################
            embedding_addr = os.path.join("build", r"premodel_+minc2_n3_sg0_hs1.txt")
            import gensim
            w2v_model = gensim.models.KeyedVectors.\
                        load_word2vec_format(fname=embedding_addr)
            tmp_embed = np.zeros((n_tokens_vocab, projection_dim), dtype=DTYPE)
            words = w2v_model.vocab
            for i, word in enumerate(words):
                tmp_embed[i+3][:] = w2v_model[word]
            ...
            self.embedding_weights = tf.get_variable(
                ...
                dtype=DTYPE,
                initializer=tf.constant_initializer(tmp_embed),
                trainable=True
            )
